...
probability_t * readBinaryFile(char *fileName, int flag){
    ...
    short previousBits = 0;
    FILE *out = fopen("t.txt", "wb");
    if(in!=NULL){
...
//                    probability[index].bits = (probability[index].bits << 4) | (buffer[1] >> 4);
                    probability[index+1].bits = (buffer[1] << 4) & mask;
                    probability[index+1].amount = 1;
                }else{
                    temp = (probability[index].bits << 4) | (buffer[0] & mask);
//                    probability[index].bits = (probability[index].bits << 4) | (buffer[0] & mask);
